...
class EmoActCvae_woEmo(nn.Module):
    def __init__(self,vocab,emo_number,act_number, model_file_path = None, is_eval = False, load_optim = False):
        ...
        self.trans_z = nn.Linear(args.hidden_dim + args.latent_dim, args.hidden_dim)
    ...
    def forward(self, batch):
        ...
        emo_clasifi = act_encoder_outputs
        ...
        return enc_batch, dec_batch, enc_batch_extend_vocab, extra_zeros, dec_ext_batch, mask_src, r_encoder_outputs, act_encoder_outputs, emo_clasifi, k_scores
    ...
    def train_one_batch(self, batch, iter, train = True):
        ...
        mask_src, r_encoder_outputs, act_encoder_outputs, emo_clasifi, k_scores = self.forward(batch)

        #torch.Size([16, 33])
        emotion_s_logit_prob = self.emo_s_fc(emo_clasifi[:, 0], train = train)
        ...
        emo_clasifi, _, _, _, _ = self.emo_attention((emo_clasifi, pred_emotion_s_embedding.unsqueeze(1).expand(pred_emotion_s_embedding.size(0),emo_clasifi.size(1),pred_emotion_s_embedding.size(1)), emo_clasifi, emo_clasifi, mask_src))

        if train:
            emotion_s_kld_loss, z_s = self.emo_s_latent_layer(emo_clasifi[:, 0], r_encoder_outputs[:, 0], train = True)
        else:
            emotion_s_kld_loss, z_s = self.emo_s_latent_layer(emo_clasifi[:, 0], r_encoder_outputs[:, 0], train = False)

        act_logit_prob = self.act_fc(act_encoder_outputs[:, 0], train = train)
        act_loss = self.act_criterion(act_logit_prob, batch['act_label'].cuda())
        softmax_a_logit = F.log_softmax(act_logit_prob, dim=-1)
        pred_act_embedding = torch.matmul(softmax_a_logit, self.act_embedding.weight)
        ...
        if not train:
            ...
            pred_act = np.argmax(softmax_a_logit.detach().cpu().numpy(), axis=1)
            act_acc = accuracy_score(batch['act_label'].cpu(), pred_act)
        ...
        sos_act_vector = torch.cat((input_vector[:, 0], selected_attribute_embedding),dim=-1)
        sos_act_vector = self.trans_input(sos_act_vector)
        input_vector = torch.cat((sos_act_vector.unsqueeze(1), input_vector[:, 1:]), dim=1)

        pre_logit, attn_dist = self.decoder(input_vector, act_encoder_outputs, k_scores, (mask_src, mask_trg))
        torch.set_printoptions(profile="full")
        # print(attn_dist)
        z_s_expand = z_s.unsqueeze(1).expand(z_s.size(0), pre_logit.size(1), z_s.size(1))
        pre_logit = torch.cat([pre_logit, z_s_expand], dim=-1)
        pre_logit = self.trans_z(pre_logit)

        logit = torch.mean(pre_logit,1)
        att = torch.matmul(logit, act_encoder_outputs.transpose(1,2))
        print(att)
        ...
        z = z_s
        z_logit = self.bow_softmax(z, both = False)
        ...
        kld_loss = emotion_s_kld_loss
        weighted_kld_loss = kl_weight * kld_loss

        elbo = loss_rec + weighted_kld_loss
        loss = loss_rec + weighted_kld_loss + 0.1*loss_aux + 0.03*emotion_s_loss + 0.05*act_loss

        emotion_l_loss = torch.tensor([0])
        emotion_l_acc = 0
    ...
    def decoder_greedy(self, batch, train=False, max_dec_step = 50):
        ...
        for i in range(max_dec_step + 1):
            ...
            z_s_expand = z_s.unsqueeze(1).expand(z_s.size(0), out.size(1), z_s.size(1))
            out = torch.cat([out, z_s_expand], dim=-1)
